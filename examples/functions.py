...
def dxf_extract_ezdxf_lines(dxf_path:str) -> List[Tuple[float,float,int,str,float]]:
...
def sample_pcd_from_linesets(linesets:List[o3d.geometry.LineSet],step_size:float=0.1)-> Tuple[o3d.geometry.PointCloud,np.ndarray]:
    ...
    for i,lineset in enumerate(linesets):
        ...
        point_cloud.paint_uniform_color(color) #! mistake
...
def ezdxf_lines_to_open3d_linesets(ezdxf_lines:List[Tuple[float,float,int,str,float]]) -> List[o3d.geometry.LineSet]: #
...
def get_uv_coordinates_from_3d(points:np.ndarray,n:ImageNode)->List[np.ndarray]:

    # transform world coordinates to imageCoordinates
    imageCoordinates= np.linalg.inv(n.cartesianTransform) @ np.append(points,1).T
